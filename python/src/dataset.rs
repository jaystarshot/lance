...
pub fn get_write_params(options: &Bound<'_, PyDict>) -> PyResult<Option<WriteParams>> {
    let params = if options.is_none() {
        ...
        if let Some(data_path_uris) = get_dict_opt::<Vec<String>>(options, "data_path_uris")? {
            println!("🐍 Python -> Rust: data_path_uris = {:?}", data_path_uris);
            p.data_path_uris = Some(data_path_uris);
        } else {
            println!("🐍 Python -> Rust: no data_path_uris provided");
        }

        if let Some(target_path_uri) = get_dict_opt::<String>(options, "target_path_uri")? {
            println!("🐍 Python -> Rust: target_path_uri = {:?}", target_path_uri);
            p.target_path_uri = Some(target_path_uri);
        } else {
            println!("🐍 Python -> Rust: no target_path_uri provided");
        }
