...
impl BTreeIndex {
    ...
    /// Bulk search interface that groups queries by pages and loads each page exactly once
    ///
    /// Instead of processing queries sequentially, this method:
    /// 1. Groups queries by the pages they need  
    /// 2. Loads each unique page once
    /// 3. Runs all relevant queries against each loaded page
    /// 
    /// This eliminates redundant cache lookups and Arc cloning in indexed joins.
pub async fn search_bulk(
    ...
    metrics: &dyn MetricsCollector,
        ...
        // Step 1: Group queries by the pages they need
        let mut pages_to_queries: HashMap<u32, Vec<usize>> = HashMap::new();
        
        for (query_idx, query) in queries.iter().enumerate() {
            let pages = self.get_pages_for_query(query)?;
        ...
        // Step 2: Initialize results for each query
        let mut results: Vec<RowIdTreeMap> = vec![RowIdTreeMap::default(); queries.len()];
        let lazy_index_reader = LazyIndexReader::new(self.store.clone());

        // Step 3: For each page, load once and run all queries that need it
        for (page_num, query_indices) in pages_to_queries {
            // Load this page exactly once
            let subindex = self.lookup_page(page_num, lazy_index_reader.clone(), metrics).await?;
            
            // Run all queries that need this page
            for &query_idx in &query_indices {
                let query = &queries[query_idx];
                
                // let page_result = match subindex.search(query, metrics).await? {
                //     SearchResult::Exact(map) => map,
                ...
                // TODO: Implement bulk search in subindex
                
                // Accumulate results for this query
                results[query_idx] |= page_result;
            }
        }
    ...
    fn get_pages_for_query(&self, query: &SargableQuery) -> Result<Vec<u32>> {
